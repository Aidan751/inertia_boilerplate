...
class OrderController extends Controller
{
    ...
    {
        $group_deal_single_item = GroupDealSingleItem::where(
            "menu_item_id",
            $id
        )->first();
        ...
        $group_deal->load(
            "groupDealItems.groupDealSingleItems.menuItem.extras"
        );
    ...
    public function placeOrder(Request $request)
    {
        ...
        if ($restaurant == null) {
            ...
            $applicationFeeAmount = round(
                ($amount - $deliveryFeeAmount) / $percentageTransaction,
                2
            );
            ...
            if (count(session("restaurant")->company_drivers) > 0) {
                ...
                $applicationFeeAmount =
                    $applicationFeeAmount + $deliveryFeeAmount;
            }
            ...
            $order = Order::create([
                ...
                "pickup_date" => date(
                    "Y-m-d",
                    strtotime(session("restaurant")->time_slot)
                ),
                ...
                "customer_contact_number" => session("restaurant")
                    ->customer_contact_number,
                "call_centre_id" => Auth::user()->id,
                "user_id" => Auth::user()->id,
                "address_line_1" =>
                    session("restaurant")->address_line_1 ?? null,
                    ...
                    "price_data" => [
                        ...
                        "product_data" => [
                            ...
                            "dietary_requirements" =>
                                $item["menu_item"]["dietary_requirements"],
            ...
            foreach ($itemsArray as $item) {
                ...
                foreach (
                    $item["price_data"]["product_data"]["sizes"]
                    as $size
                ) {
                ...
                foreach (
                    $item["price_data"]["product_data"]["extras"]
                    as $extra
                ) {
            ...
            $order->save();
            $twilio = TwilioPackage::sendSMS(
                $order->customer_contact_number,
                "To complete your OrderIt order, please make your payment at the following URL: " .
                    $session->url
            );
            ...
            if (isset($session->url)) {
                return redirect()
                    ...
                    ->with("stripe_url", $session->url);
            }

            return redirect()
                ->route("call-centre.orders.search", ["id" => Auth::user()->id])
                ->with("success", "Order placed and SMS sent.");
        }
    }
    ...
    {
        $group_deal = session("group_deal")->load(
            "groupDealItems.groupDealSingleItems.menuItem"
        );
        $restaurant = session("restaurant");
        $group_deal_single_item = session("group_deal_single_item");
        $menu_item = MenuItem::where(
            "id",
            $group_deal_single_item->menu_item_id
        )->first();
        $new_selected_size = [];
        $new_selected_extra = [];
        $notes = "";
        ...
        if ($menu_item_sizes) {
            foreach ($menu_item_sizes as $size) {
                if ($size["id"] == $request->size) {
                    $size["additional_charge"] = floatval(
            ...
            foreach ($menu_item_extras as $extra) {
                foreach ($request->extra as $extra_id) {
                    if ($extra["id"] == $extra_id) {
                        $extra["additional_charge"] = floatval(
        ...
        return Inertia::render("CallCentreAdmin/Orders/AddDeal", [
            "groupDeal" => $group_deal->load(
                "groupDealItems.groupDealSingleItems.menuItem"
            ),
            "restaurant" => $restaurant,
            "selected_items" => $deal_selected_items,
            "message" =>
                "Successfully added size and extras to item " .
                $menu_item->name,
        ]);
    }
    ...
    public function addToBasket(Request $request)
    {
        ...
        return Inertia::render("CallCentreAdmin/Orders/Index", [
            ...
            "restaurant" => $restaurant,
            "group_deal" => $group_deal,
            "deal_selected_items" => $deal_selected_items,
    ...
    public function showOrder(Request $request)
    {
        ...
        if (
            ...
            return redirect()
                ...
                ->withErrors([
                    "error",
                    "This business does not offer a delivery service.",
                ]);
        }

        if (
            $request->order_type == "collection" &&
            ...
            return redirect()
                ...
                ->withErrors([
                    "error",
                    "This business does not offer a collection service.",
                ]);
        }

        if (
            $request->order_type == "table" &&
            ...
            return redirect()
                ...
                ->withErrors([
                    "error",
                    "This business does not offer table service.",
                ]);
        }
        ...
        if ($request->when_radio == "asap") {
            ...
            $selected_time = date_format(
                new DateTime($request->selected_time),
                "H:i:s"
            );
        }
        ...
        $todaysHours = OpeningHour::where("day_id", $day->id)
        ...
        if (count($todaysHours) > 0) {
            ...
            foreach ($todaysHours as $hours) {
                ...
                if (
                    $selected_time >=
                        date_format(new DateTime($hours->from), "H:i:s") &&
                    $selected_time <
                        date_format(new DateTime($hours->to), "H:i:s")
                ) {
                    // Business is open
                    $businessClosed = false;
                    $openingHoursMessage =
                        "Open till " .
                        date_format(new DateTime($hours->to), "H:i");
                } else {
                    $businessClosed = true;
                }
                if ($businessClosed) {
                    ...
                    if (
                        ...
                        $openingHoursMessage =
                            "Opens at " .
                            date_format(new DateTime($hours->from), "H:i");
                    } elseif (
                        $selected_time >
                        date_format(new DateTime($hours->to), "H:i:s")
                    ) {
        ...
        if ($request->order_type == "delivery") {
            ...
            if ($decodedResponse["status"] == "OK") {
                ...
                if (
                    !empty($decodedResponse["results"][0]["address_components"])
                ) {
                ...
                return redirect()
                    ...
                    ->withErrors([
                        "message",
                        "Address not found, please check this is a valid address.",
                    ]);
            } else {
                return redirect()
                    ->back()
            ...
            if (!is_null($restaurant)) {
                ...
                if ($request->order_type == "delivery") {
                    ...
                    if ($decodedResponse["status"] == "OK") {
                        ...
                        $user_latitude =
                            $decodedResponse["results"][0]["geometry"][
                                "location"
                            ]["lat"];
                        $user_longitude =
                            $decodedResponse["results"][0]["geometry"][
                ...
                // test
                if (
                    count($restaurant->company_drivers) > 0 &&
                    $request->order_type == "delivery"
                ) {
                    $configurations = Configuration::get()->toArray();

                    $distance_in_miles = GeocoderPackage::getDistance(
                        $userAddress,
                        $restaurant->getFullAddressAttribute()
                    );

                    $raw_distance = (int) str_replace(
                        " miles",
                        "",
                        $distance_in_miles
                    );
                    $delivery_time = GeocoderPackage::getDeliveryTime(
                        $restaurant->getFullAddressAttribute(),
                        $userAddress
                    );
                    ...
                    $fee =
                        $time_in_minutes * (int) $configurations[0]["minute"] +
                        $raw_distance * (int) $configurations[0]["mile"];
                ...
                foreach ($group_deals as $deal) {
                    $deal
                        ->load(
                            "groupDealItems.groupDealSingleItems.menuItem.sizes",
                            "groupDealItems.groupDealSingleItems.menuItem.extras"
                        )
                        ->get();
                }
                $menu_categories = MenuCategory::where(
                    "restaurant_id",
                    $restaurant->id
                )->get();
                foreach ($menu_categories as $menu_category) {
                    $menu_category->menu_items = MenuItem::where(
                        "menu_category_id",
                        $menu_category->id
                    )
                        ->with("extras", "sizes")
                ...
                $restaurant->setAttribute(
                    "customer_contact_number",
                    $request->customer_contact_number
                );
                $restaurant->setAttribute(
                    "time_slot",
                    date_format(new DateTime($selected_time), "H:i:s")
                ) ?? null;
                ...
                return redirect()
                    ...
                    ->withErrors([
                        "error",
                        "Business not found, please check the businesses number is correct.",
                    ]);
            }
        }
    }
        ...
        if ($order == null) {
            ...
            if ($request->accept == "accept") {
                ...
                if ($capture->status == "succeeded") {
                    ...
                    app("App\Services\PushNotification")->sendPush(
                        config("app.name"),
                        "Your order from $name has been accepted and payment has been taken",
                        [$order->customer_id],
                        "orders",
                        $order->id
                    );
                    ...
                    app("App\Services\PushNotification")->sendPush(
                        config("app.name"),
                        "Your order from $name has a problem. Payment was unable to be taken.",
                        [$order->customer_id],
                        "orders",
                        $order->id
                    );
                    return redirect()
                        ->back()
                        ->with("fail", "Payment failed to be captured!");
                }
                ...
                app("App\Services\PushNotification")->sendPush(
                    config("app.name"),
                    "Your order from $name has been cancelled. You have not been charged.",
                    [$order->customer_id],
                    "orders",
                    $order->id
                );
