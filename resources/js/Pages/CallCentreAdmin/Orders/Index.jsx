...
export default function Index(props) {
  ...
  selectedDealItems.length > 0 &&
    selectedDealItems.forEach((item) => {
      item.quantity = 1;
      total_deal_item_cost_arr.push(item.group_deal_price);
      item.group_deal_items.forEach((dealItem) => {
        let total_deal_extra_cost =
          dealItem.group_deal_single_items[0].menu_item.extras &&
          dealItem.group_deal_single_items[0].menu_item.extras.length > 0 &&
          dealItem.group_deal_single_items[0].menu_item.extras.reduce(
            (prev, curr, index, array) =>
              prev + parseFloat(curr.additional_charge),
            0
          );
        ...
        let total_deal_size_cost =
          dealItem.group_deal_single_items[0].menu_item.sizes &&
          dealItem.group_deal_single_items[0].menu_item.sizes.length > 0 &&
          dealItem.group_deal_single_items[0].menu_item.sizes.reduce(
            (prev, curr, index, array) =>
              prev + parseFloat(curr.additional_charge),
            0
          );
  ...
  total_deal_extra_cost =
    total_deal_extra_cost_arr &&
    total_deal_extra_cost_arr.reduce(
      (prev, curr, index, array) => prev + parseFloat(curr),
      0
    );
  total_deal_size_cost =
    total_deal_item_cost_arr &&
    total_deal_size_cost_arr.reduce(
      (prev, curr, index, array) => prev + parseFloat(curr),
      0
    );
  total_deal_item_cost =
    total_deal_item_cost_arr &&
    total_deal_item_cost_arr.reduce(
      (prev, curr, index, array) => prev + parseFloat(curr),
      0
    );
    ...
    props.restaurant.group_deals.forEach((main_menu_item) => {
      if (main_menu_item.id === menu_item.id) {
        main_menu_item.group_deal_items.forEach((dealSingleItem, itemKey) => {
          dealSingleItem.group_deal_single_items[0].menu_item.sizes =
            sizes[itemKey];
          dealSingleItem.group_deal_single_items[0].menu_item.extras =
            extras[itemKey];
        });
      ...
      selectedItems.forEach((item) => {
        ...
        if (item.extra && item.extra.length > 0) {
          while (i < item.extra.length) {
            extra_total +=
              parseFloat(item.extra[i].additional_charge) *
      ...
      selectedItems.forEach((item) => {
        ...
        if (item.size && item.size.length > 0) {
          while (i < item.size.length) {
            size_total +=
              parseFloat(item.size[i].additional_charge) *
              parseFloat(item.menu_item.quantity);
            i++;
          }
        }
  ...
  const handleActiveObjectExtrasChange = (e, extra) => {
    ...
    if (isChecked) {
      ...
      const newExtras = activeObjectExtras.filter(
   ...
   * @param {*} extra
   */
  const handleActiveDealObjectExtrasChange = (
    event,
    extra,
    groupDealItemKey
  ) => {
    ...
    if (isChecked) {
      ...
      const newExtras = list[groupDealItemKey].filter(
        (item) => item.id !== extra.id
      );
  ...
  const onHandleDealDelete = (e, deal_item) => {
    e.preventDefault();
    const newSelectedDealItems = selectedDealItems.filter(
      (item) => item.id !== deal_item.id
    );
  ...
  return (
    <>
      <Authenticated auth={props.auth} errors={props.errors} activeGroup={16}>
            ...
            <div className="lg:col-span-2 col-span-3 sm:row-span-1">
              <div className="mb-4 sm:mb-0 flex flex-col-reverse items-start justify-start sm:grid sm:grid-cols-4 sm:grid-rows-2 sm:p-10 sm:pb-0">
                ...
                <Link
                  ...
                  href={route("call-centre.orders.search", {
              ...
              <div className="mt-5 md:col-span-2 col-span-3 sm:row-span-1 row-start-2 border-t border-b-none border-stone-400">
                <h2 className="font-medium text-lg sm:px-10 mt-8 pb-0">
                  Deals
                </h2>
                {props.restaurant.group_deals &&
                  props.restaurant.group_deals.map(
                    ({
                      title,
                      image,
                      description,
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              onHandleDealAddition(e, {
                                ...
                                price: group_deal_price,
                                group_deal_items: group_deal_items,
                              });
                            }}
                          >
                            Add
              ...
              <form onSubmit={submit}>
                <h2 className="p-5 font-medium text-lg border rounded py-3">
                  ...
                  {(selectedItems ? selectedItems.length : 0) +
                    (selectedDealItems ? selectedDealItems.length : 0)}
                  )
                </h2>
                <TabPanel>
                            ...
                            <div className="ml-auto flex items-center">
                              ...
                              <div>
                                <X
                                  ...
                                  onClick={(e) => onHandleDealDelete(e, item)}
                                />
                              </div>
                            </div>
                          </a>

                          {item.group_deal_items.map(
                            (dealItem, dealItemKey) => {
                              return (
                                <div className="mb-5">
                                  ...
                                  {dealItem.group_deal_single_items[0].menu_item
                                    .sizes &&
                                    dealItem.group_deal_single_items[0].menu_item.sizes.map(
                                      (size, sizeKey) => {
                                        return (
                                  ...
                                  {dealItem.group_deal_single_items[0].menu_item
                                    .extras &&
                                    dealItem.group_deal_single_items[0].menu_item.extras.map(
                                      (extra, extraKey) => {
                                        return (
                                          <div className="flex items-center cursor-pointer transition duration-300 ease-in-out bg-white dark:bg-darkmode-600 hover:bg-slate-100 dark:hover:bg-darkmode-400 rounded-md">
                                            - {extra.name} : £{" "}
                                            {extra.additional_charge}
                                          </div>
                                        );
                                      }
                                    )}
                      ...
                      selectedItems.map((item, key) => (
                        <div className="mb-5">
                          <a className="flex mb-5 items-center cursor-pointer transition duration-300 ease-in-out bg-white dark:bg-darkmode-600 hover:bg-slate-100 dark:hover:bg-darkmode-400 rounded-md">
                            ...
                            <Edit
                              className="w-4 h-4 text-slate-500 ml-2"
                              onClick={(e) => onHandleItemEdit(e, item)}
                            />
                            <div className="ml-auto flex items-center">
                              <div className="font-medium text-lg">
                                £{" "}
                                {parseFloat(item.menu_item.price) *
                                  ...
                                  (item.extra && item.extra.length > 0
                                    ? item.extra.reduce(
                                        (prev, curr, index, array) =>
                                          prev +
                                          parseFloat(curr.additional_charge),
                                        0
                                      )
                                    : 0) *
                              ...
                              <div>
                                <X
                                  className="w-4 h-4 text-slate-500 ml-2"
                                  onClick={(e) => onHandleItemDelete(e, item)}
                                />
                              </div>
                            </div>
                          </a>
                      ...
                      <div className="flex mt-4 pt-4 border-t border-stone-400 dark:border-darkmode-400">
                        ...
                        <div className="font-medium text-base">
                          £
                          {
                            (main_total =
                              parseFloat(total_price) +
                              (parseFloat(props.restaurant.delivery_charge) ||
                                0) +
                              (parseFloat(props.restaurant.service_charge) ||
                                0) +
                              parseFloat(extra_total) +
                              parseFloat(size_total) +
                              (parseFloat(grand_total_for_deals) || 0))
                          }
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-col mt-5">
                    ...
                    <button
                      className="btn w-full mt-2 border-slate-300 dark:border-darkmode-400 text-slate-500"
                      onClick={clearItems}
                    >
                      Clear Items
                    </button>
            ...
            <ModalHeader>
              <div className="flex flex-col pt-0 p-5">
                ...
                <div className="col-span-12 mt-5">
                  ...
                  <p className="mt-2">
                    {activeObject.dietary_requirements ?? ""}
                  </p>
                        ...
                        {size.name && (
                          <div className="flex items-center mt-5">
                            <input
                              ...
                              name="size"
                              value={size.id}
                              onChange={handleActiveObjectSizeChange}
                            />{" "}
                          ...
                          <div className="flex items-center mt-0">
                            <div className="form-check mb-2">
                              <input
                                ...
                                value={extra.id}
                                onChange={(e) =>
                                  handleActiveObjectExtrasChange(e, extra)
                                }
                              />
                              <label
              ...
              <div className="flex flex-col pt-0 p-5">
                ...
                <img
                  src={
                    activeEditItemObject &&
                    activeEditItemObject.menu_item &&
                    activeEditItemObject.menu_item.image
                  }
                  alt={
                    activeEditItemObject &&
                    activeEditItemObject.menu_item &&
                ...
                <div className="col-span-12 mt-5">
                  <p>
                    {activeEditItemObject &&
                      activeEditItemObject.menu_item &&
                      activeEditItemObject.menu_item.description}
                  </p>
                  <p className="mt-2">
                    {activeEditItemObject &&
                      activeEditItemObject.menu_item &&
                      activeEditItemObject.menu_item.dietary_requirements}
                  </p>
                  <p className="mt-2">
                    £
                    {activeEditItemObject &&
                      activeEditItemObject.menu_item &&
                      activeEditItemObject.menu_item.price}
                  </p>
                </div>
              </div>
            </ModalHeader>
            <ModalBody>
              ...
              <div className="w-full p-5 flex justify-between items-start">
                <div>
                  ...
                  {activeEditItemObject &&
                    activeEditItemObject.menu_item &&
                    activeEditItemObject.menu_item.sizes &&
                ...
                <div>
                  ...
                  {activeEditItemObject &&
                    activeEditItemObject.menu_item &&
                    activeEditItemObject.menu_item.extras &&
                          ...
                          <div className="flex items-center mt-0">
                            <div className="form-check mb-2">
                              <input
                                ...
                                value={extra.id}
                                onChange={(e) =>
                                  handleEditObjectExtrasChange(e, extra)
                                }
                              />
                              <label
                                className="form-check-label flex"
          ...
          >
            <ModalHeader>
              <div className="flex flex-col pt-0 p-5">
                ...
                <img
                  src={activeDealObject.image ?? ""}
                  alt={activeDealObject.title ?? ""}
                  className="rounded-md"
                />
                <div className="col-span-12 mt-5">
                  <p>{activeDealObject.description ?? ""}</p>
                  <p className="mt-2">
                    {activeDealObject.dietary_requirements ?? ""}
                  </p>
            ...
            <ModalBody>
              {activeDealObject.group_deal_items &&
                activeDealObject.group_deal_items.map((item, itemKey) => {
                  return (
                    <>
                      <div className="flex flex-col pt-0 p-5">
                        <h2 className="font-medium text-base mr-auto mb-5 mt-5">
                        ...
                        <img
                          src={
                            item.group_deal_single_items[0].menu_item.image ??
                            ""
                          }
                          alt={
                            item.group_deal_single_items[0].menu_item.title ??
                        ...
                        <div className="col-span-12 mt-5">
                          <p>
                            {item.group_deal_single_items[0].menu_item
                              .description ?? ""}
                          </p>
                          <p className="mt-2">
                            {item.group_deal_single_items[0].menu_item
                          ...
                          <p className="mt-2">
                            £
                            {item.group_deal_single_items[0].menu_item.price ??
                              ""}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="w-full p-5 flex justify-between items-start">
                        <div>
                          ...
                          {item.group_deal_single_items[0].menu_item.sizes &&
                            item.group_deal_single_items[0].menu_item.sizes.map(
                              (size, key) => (
                                <div key={key}>
                                  {size.name && (
                                    <div className="flex items-center mt-5">
                                      <input
                                        type="radio"
                                        name={
                                          "group_deal_size_select-" + item.id
                                        }
                                        value={
                                        ...
                                        onChange={(e) =>
                                          handleActiveDealObjectSizeChange(
                                            itemKey,
                                            size
                                          )
                                        }
                        ...
                        <div>
                          ...
                          {item.group_deal_single_items[0].menu_item.extras &&
                            item.group_deal_single_items[0].menu_item.extras.map(
                              (extra, key) => (
                                <div key={key}>
                                  {extra.name && (
                                    <div className="flex items-center mt-0">
                                      <div className="form-check mb-2">
                                        <input
                                          ...
                                          value={extra.id}
                                          onChange={(e) =>
                                            handleActiveDealObjectExtrasChange(
                                              e,
                                              extra,
                                              itemKey
                                            )
        ...
        >
          <div className="font-medium">Yay! Menu Item Added!</div>
        </Notification>
