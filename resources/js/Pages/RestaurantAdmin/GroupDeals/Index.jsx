...
export default function Index(props){
    ...
    const from = props.groupDeals.from;

    const to = props.groupDeals.to;

    const total = props.groupDeals.total;

    const first_page_url = props.groupDeals.first_page_url;

    const last_page_url = props.groupDeals.last_page_url;

    const links = props.groupDeals.links;
    ...
    */
    function handleSearch(e) {
        e.preventDefault();
        Inertia.get(route('restaurant.group-deals.index'), data);
    }
    ...
    const deleteRecord = (e) => {
        Inertia.delete(route('restaurant.group-deals.destroy',{id:deleteId}),{
            preserveState: false,
            onSuccess: () => {
            }
        });
    ...
    const paginate = (e) => {
        e.preventDefault();
        Inertia.get(route('restaurant.group-deals.index'), {
    ...
    return (
        <>
            ...
            >
                ...
                <main className="col-span-12">
                    ...
                    {/* Page Header */}
                    <h2 className="intro-y text-lg font-medium mt-10">Manage Group Deals</h2>
        ...
        <Link href={route("restaurant.group-deals.create", {id: props.auth.user.restaurant_id})} className="btn btn-primary shadow-md mb-5" style={{whiteSpace: "nowrap"}}>
                        ...
                        <table className="table table-report -mt-2">
                            ...
                            <tbody>
                            { props.groupDeals.data.map((groupDeal, key) => (

                                <tr key={key} className="intro-x">

                                    {/* Menu groupDeal Title */}
                                    <td className="text-left">
                                     ...
                                     <td className="table-report__action w-56">
                                        <div className="flex justify-center groupDeals-center">
                                            ...
                                            {/* Delete Link */}
                                            <button
                                                className="flex groupDeals-center text-danger"
                                                type="button"
                                                onClick={setDeleteConfirmationModal}
                                                id={groupDeal.id}
                                            >
                                                <Trash2 id={groupDeal.id} className="w-4 h-4 mr-1" /> Delete
                                            </button>
                                        </div>
                                    </td>
                                </tr>
